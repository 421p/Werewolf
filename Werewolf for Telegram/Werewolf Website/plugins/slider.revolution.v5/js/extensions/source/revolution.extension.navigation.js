            ...
            createNavigation: function(container, opt) {
                ...
                container.bind('revolution.slide.onafterswap revolution.nextslide.waiting',
                    function() {
                        ...
                        var si = container.find(".next-revslide").length == 0
                        ...
                        if (_a.enable === true) {
                            ...
                            jQuery.each(opt.thumbs[pi].params,
                                function(i, obj) {
                            ...
                            jQuery.each(opt.thumbs[ni].params,
                                function(i, obj) {
                                    inst = inst.replace(obj.from, obj.to);
                                });
...
/////////////////////////////////
    ...
    var moveThumbsInPosition = function(container, opt) {
        ...
        if (me > 2) {
            ...
            curpos = ((tp - tw + elp + tmw) < tw && tp + (Math.round(me) - 2) * tw < elp)
        ...
        if (!container.hasClass("dragged")) {
            if (thumbdir === "vertical")
                el.data('tmmove',
                    punchgs.TweenLite.to(el, 0.5, { top: curpos + "px", ease: punchgs.Power3.easeInOut }));
            else
                el.data('tmmove',
                    punchgs.TweenLite.to(el, 0.5, { left: curpos + "px", ease: punchgs.Power3.easeInOut }));
...
// INTERNAL FUNCTIONS
    ...
    var initMouseScroll = function(container, opt) {
        ...
        if (opt.navigation.mouseScrollNavigation !== "on" &&
            opt.navigation.mouseScrollNavigation !== "carousel") return;
        ...
        container.on('mousewheel DOMMouseScroll',
            function(e) {
                ...
                var res = normalizeWheel(e.originalEvent),
                    ...
                    ls = asi != -1 && asi == opt.slideamount - 1 || psi != 1 && psi == opt.slideamount - 1
                        ? true
                        : false,
                    ret = true;
    ...
    var isme = function(a, c, e) {
        a = _ISM
            ? jQuery(e.target).closest('.' + a).length || jQuery(e.srcElement).closest('.' + a).length
            : jQuery(e.toElement).closest('.' + a).length || jQuery(e.originalTarget).closest('.' + a).length;
...
// 	-	SET THE SWIPE FUNCTION //	
    var swipeAction = function(container, opt, vertical) {
        ...
        var notonbody = _ISM || _R.get_browser() === "Firefox",
            ...
            pagescroll =
                opt.navigation.thumbnails.direction === "vertical" || opt.navigation.tabs.direction === "vertical"
                    ? "none"
                    : "vertical",
            swipe_wait_dir = opt.navigation.touch.swipe_direction || "horizontal";

        pagescroll = vertical == "swipebased" && swipe_wait_dir == "vertical"
            ? "none"
            : vertical
            ? "vertical"
            : pagescroll;
        ...
        SwipeOn.swipetp({
            ...
            swipeStatus: function(event, phase, direction, distance, duration, fingerCount, fingerData) {
                ...
                (((phase === "move" || phase === "end" || phase == "cancel") &&
                    ...
                    distance = (direction && direction.match(/left|up/g))
                    ...
                    case "start":
                        if (_.positionanim !== undefined) {
                            ...
                            _.slide_globaloffset =
                                _.infinity === "off" ? _.slide_offset : _R.simp(_.slide_offset, _.maxwidth);
                        }
                        _.overpull = "none";
                    ...
                    case "move":
                        ...
                        if (_.infinity === "off") {
                            var bb = _.horizontal_align === "center"
                ...
                if ((
                    ...
                    case "move":
                        ...
                        curpos = curpos > 0
                            ? tdir === "horizontal"
                            ? curpos - (el.width() * (curpos / el.width() * curpos / el.width()))
                            : curpos - (el.height() * (curpos / el.height() * curpos / el.height()))
                            : curpos;
                        var dif = tdir === "vertical" ? 0 - (el.height() - t.height()) : 0 - (el.width() - t.width());
                        curpos = curpos < dif
                            ? tdir === "horizontal"
                            ? curpos + (el.width() * (curpos - dif) / el.width() * (curpos - dif) / el.width())
                            : curpos + (el.height() * (curpos - dif) / el.height() * (curpos - dif) / el.height())
                            : curpos;
                        if (tdir === "vertical")
                    ...
                    if (phase == "end" && !istt) {
                        ...
                        if ((swipe_wait_dir == "horizontal" && direction == "left") ||
                            (swipe_wait_dir == "vertical" && direction == "up")) {
                            opt.sc_indicator_dir = 0;
                            _R.callingNewSlide(opt, opt.c, 1);
                            return false;
                        }
                        if ((swipe_wait_dir == "horizontal" && direction == "right") ||
    ...
    var callAllDelayedCalls = function(container, opt) {
        ...
        if (ckNOLO(opt.navigation.arrows))
            punchgs.TweenLite.delayedCall(
                _ISM ? opt.navigation.arrows.hide_delay_mobile : opt.navigation.arrows.hide_delay,
                showHideNavElements,
                [cp.find('.tparrows'), opt.navigation.arrows, "hide"]);

        if (ckNOLO(opt.navigation.bullets))
            punchgs.TweenLite.delayedCall(
                _ISM ? opt.navigation.bullets.hide_delay_mobile : opt.navigation.bullets.hide_delay,
                showHideNavElements,
                [cp.find('.tp-bullets'), opt.navigation.bullets, "hide"]);

        if (ckNOLO(opt.navigation.thumbnails))
            punchgs.TweenLite.delayedCall(
                _ISM ? opt.navigation.thumbnails.hide_delay_mobile : opt.navigation.thumbnails.hide_delay,
                showHideNavElements,
                [cp.find('.tp-thumbs'), opt.navigation.thumbnails, "hide"]);

        if (ckNOLO(opt.navigation.tabs))
            punchgs.TweenLite.delayedCall(_ISM ? opt.navigation.tabs.hide_delay_mobile : opt.navigation.tabs.hide_delay,
                showHideNavElements,
...
// ADD ARROWS
    var initArrows = function(container, o, opt) {
        ...
        if (container.find('.tp-leftarrow.tparrows').length === 0)
            container.append('<div class="tp-leftarrow tparrows ' +
                o.style +
                ' ' +
                o.left.style +
                '">' +
        ...
        if (container.find('.tp-rightarrow.tparrows').length === 0)
            container.append('<div class="tp-rightarrow tparrows ' +
                o.style +
                ' ' +
                o.right.style +
                '">' +
...
// PUT ELEMENTS VERTICAL / HORIZONTAL IN THE RIGHT POSITION
    var putVinPosition = function(el, o, opt) {
        ...
        var elh = el.outerHeight(true),
            ...
            by = o.container == "layergrid"
                ? opt.sliderLayout == "fullscreen"
                ? opt.height / 2 - (opt.gridheight[opt.curWinRange] * opt.bh) / 2
                : (opt.autoHeight == "on" || (opt.minHeight != undefined && opt.minHeight > 0))
...
// ADD THUMBNAILS
    var addThumb = function(container, o, li, what, opt) {
        ...
        var linkto = li.data('index'),
            ...
            tm = t.find(thumbmask),
            tw = tm.find(thumbsiw),
            maxw = o.direction === "horizontal"
                ? (o.width * o.visibleAmount) + (o.space * (o.visibleAmount - 1))
                : o.width,
            maxh = o.direction === "horizontal"
                ? o.height
                : (o.height * o.visibleAmount) + (o.space * (o.visibleAmount - 1)),
            inst = o.tmp;
        jQuery.each(opt.thumbs[li.index()].params,
            function(i, obj) {
                inst = inst.replace(obj.from, obj.to);
            })


        tw.append('<div data-liindex="' +
            li.index() +
            '" data-liref="' +
            linkto +
        ...
        var position = o.position === "outer-top" || o.position === "outer-bottom" ? "relative" : "absolute",
            _margin = (o.position === "outer-top" || o.position === "outer-bottom") && (o.h_align === "center")
                ? "auto"
                : "0";
