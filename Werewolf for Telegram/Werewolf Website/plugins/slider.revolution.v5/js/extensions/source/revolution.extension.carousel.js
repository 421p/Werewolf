        ...
        _R,
        {
            ...
            carouselToEvalPosition: function(opt, direction) {
                ...
                calc = cm >= 0.3 && direction === "left" || cm >= 0.7 && direction === "right"
                    ? mc
                    : cm < 0.3 && direction === "left" || cm < 0.7 && direction === "right"
                    ? mf
                    : calc;
                calc = _.infinity === "off"
                    ? fi < 0
            ...
            organiseCarousel: function(opt, direction, setmaind, unli) {
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    if (_.infinity === "on") {
                        pos = pos > _.wrapwidth - _.inneroffset && direction == "right"
                ...
                if (_.slides)
                    jQuery.each(_.slides,
                        function(i, slide) {
                            var pos = slidepositions[i];
                            if (_.infinity === "on") {

                                pos = pos > _.wrapwidth - _.inneroffset && direction === "left"
                            ...
                            var d = _.horizontal_align === "center"
                            ...
                            if (_.fadeout === "on")
                                if (_.vary_fade === "on")
                                    ...
                                    case "center":
                                        tr.autoAlpha = Math.abs(d) < Math.ceil((_.maxVisibleItems / ha) - 1)
                                            ? 1
                                            : 1 - (Math.abs(d) - Math.floor(Math.abs(d)));
                                        break;
                                    case "left":
                                        tr.autoAlpha = d < 1 && d > 0
                                            ? 1 - d
                            ...
                            if (_.minScale !== undefined && _.minScale > 0) {
                                if (_.vary_scale === "on") {
                                    tr.scale = 1 -
                                        Math.abs(((_.minScale / 100 / Math.ceil(_.maxVisibleItems / ha)) * d));
                            ...
                            if (_.maxRotation !== undefined && Math.abs(_.maxRotation) != 0) {
                                if (_.vary_rotation === "on") {
                                    tr.rotationY =
                                        ...
                                        Math.abs((1 - Math.abs(((1 / Math.ceil(_.maxVisibleItems / ha)) * d))) *
                                            _.maxRotation);
                            ...
                            // TRANSFORM STYLE			
                            tr.transformStyle = opt.parallax.type != "3D" && opt.parallax.type != "3d"
                                ? "flat"
                                : "preserve-3d";
    ...
    var setCarouselDefaults = function(opt) {
        ...
        if (_.horizontal_align === "right")
            punchgs.TweenLite.set(_.wrap,
                { left: "auto", right: _.wrapoffset + "px", width: _.wrapwidth, overflow: ovf });
        else
            punchgs.TweenLite.set(_.wrap,
                { right: "auto", left: _.wrapoffset + "px", width: _.wrapwidth, overflow: ovf });
...
// ANIMATE THE CAROUSEL WITH OFFSETS
    var animateCarousel = function(opt, direction, nsae) {
        ...
        _.positionanim = punchgs.TweenLite.to(animobj,
            1.2,
            {
                from: animobj.to,
                onUpdate: function() {
