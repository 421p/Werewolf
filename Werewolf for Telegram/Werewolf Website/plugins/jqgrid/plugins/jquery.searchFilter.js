...
jQuery.fn.searchFilter = function(fields, options) {
    ...
    function SearchFilter(jQ, fields, options) {
        ...
        if (fields != null) { // type coercion matches undefined as well as null
            ...
            jQ.html("").addClass("ui-searchFilter").append(
                    ...
                    if (this.dataUrl != null) {
                        ...
                        jQuery.ajax(jQuery.extend({
                                ...
                                complete: function(data) {
                                    ...
                                    ) { // change should get called no more than twice when this searchFilter is constructed
            ...
            // bind events
            jQ.find(".ui-state-default").hover(hover, hover).mousedown(active)
                .mouseup(active); // add hover/active effects to all buttons
            ...
            jQ.find(".ui-del").click(function(e) {
                ...
                if (row.siblings(".sf").length > 0) { // doesn't remove if there's only one filter left
                    ...
                    row.find("select[name='field']")
                        .change(function(event) { event.stopPropagation(); }); // trigger any change events
                }
                return false;
            });
            jQ.find(".ui-add").click(function(e) {
                ...
                if (opts.clone) {
                    newRow.find("select[name='field']")[0].selectedIndex =
                        row.find("select[name='field']")[0].selectedIndex;
                    var stupid_browser = (newRow.find("select[name='op']")[0] == null); // true for IE6
                    if (!stupid_browser)
                        newRow.find("select[name='op']").focus()[0].selectedIndex =
                            row.find("select[name='op']")[0].selectedIndex;
                ...
                if (opts.datepickerFix === true && jQuery.fn.datepicker !== undefined
                ) { // using $.data to associate data with document elements is Not Good
                    row.find(".hasDatepicker").each(function() {
                        var settings = jQuery.data(this, "datepicker").settings;
                        newRow.find("#" + this.id).unbind().removeAttr("id").removeClass("hasDatepicker")
                            .datepicker(settings);
                    });
                }
            ...
            this.setFilter = function(settings) {
                ...
                for (i = 0, li = fields.length; i < li; i++) {
                    ...
                    if (selDOMobj) {
                        valueindexmap[fields[i]]['data'] =
                            {}; // this setting is the flag that 'data' is contained in a SELECT
                        for (j = 0, lj = selDOMobj.options.length; j < lj; j++) {
                ...
                if (fieldindex != null && opindex != null && dataindex != null) {
                    ...
                    o.find("input.vdata")
                        .val(datavalue); // if jquery does not find any INPUT, it does not set any. This means we deal with SELECT
